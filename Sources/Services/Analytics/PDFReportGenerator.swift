...
final class PDFReportGenerator {
    ...
    func generateReport(type: ReportItem, reportType: ReportListView.ReportType, stations: [NetworkStation], pipelines: [Pipeline], alerts: [Alert]) -> Data {
        ...
        case "Network Summary":
            content += generateNetworkSummary(stations: stations, pipelines: pipelines, alerts: alerts)
        case "Pressure Analysis":
            content += generatePressureAnalysis(stations: stations)
        case "Flow Report":
            content += generateFlowReport(stations: stations)
        case "Alert History":
            content += generateAlertHistory(alerts: alerts)
        case "Maintenance Log":
            content += generateMaintenanceLog()
        case "Asset Inventory":
            content += generateAssetInventory(stations: stations, pipelines: pipelines)
        case "Safety Report":
            content += generateSafetyReport(alerts: alerts)
        case "Cost Analysis":
            content += generateCostAnalysis()
        default:
            content += "Report data available upon request."
        }
    ...
    private func generateFlowReport(stations: [NetworkStation]) -> String {
        ...
        let avgFlow = totalFlow / Double(flows.count)
